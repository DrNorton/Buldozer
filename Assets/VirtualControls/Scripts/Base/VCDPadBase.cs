...
public abstract class VCDPadBase : MonoBehaviour 
{
	...
	{
		SetPressed(EDirection.Right, joystick.AxisX > 0.0f && XAxisEnabled && VCTouchController.InputAdjustValue > 0);
		SetPressed(EDirection.Left, joystick.AxisX < 0.0f && XAxisEnabled && VCTouchController.InputAdjustValue > 0);
		SetPressed(EDirection.Up, joystick.AxisY > 0.0f && YAxisEnabled && VCTouchController.InputAdjustValue > 0);
		SetPressed(EDirection.Down, joystick.AxisY < 0.0f && YAxisEnabled && VCTouchController.InputAdjustValue > 0); 
	}
